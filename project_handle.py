...
## Open File Process
########################################################################
def get_attr(item, path):
    xmldoc = minidom.parse(os.path.join(path, 'bin', 'empresa.xml'))
    return [x.attributes[item].value for x in xmldoc.getElementsByTagName('item')]
...
if __name__ == "__main__":
    g = []
    indexes = get_attr('indice', 'U:/AffidavitsSdeRLdeCV/')
    names = get_attr('nombre', 'U:/AffidavitsSdeRLdeCV/')
    for i, record in enumerate(indexes):
        print indexes[i], names[i]
